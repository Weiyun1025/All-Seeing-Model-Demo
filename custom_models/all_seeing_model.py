...
# from .husky_window import WindowBLIP2EncoderLayer
...
class UnifiedHuskyConfig(Blip2Config):
    def __init__(
        self,
        output_size=(7, 7),
        sampling_ratio=-1,
        ...
        lora_vision=False,
        window_size: int = 16,
        use_window_attn: Optional[bool] = None,
        **kwargs,
    ):
        ...
        self.window_attn = [False] * self.vision_config.num_hidden_layers
        self.window_attn = tuple(self.window_attn)

        self.update_window_attn()

    def update_window_attn(self):
        if self.use_window_attn:
            freq = self.vision_config.num_hidden_layers // 4
            self.window_attn = [
                (i+1) % freq != 0
                for i in range(self.vision_config.num_hidden_layers)
            ]
            self.window_attn = tuple(self.window_attn)
        else:
            self.window_attn = [False] * self.vision_config.num_hidden_layers
...
class AllSeeingModel(Blip2PreTrainedModel):
    config_class = UnifiedHuskyConfig
    main_input_name = "pixel_values"

    def __init__(self, config: UnifiedHuskyConfig):
        super().__init__(config)

        # init vision module
        self.vision_model = Blip2VisionModel(config.vision_config)

        # init window attn for vision module
        assert any(config.window_attn) == bool(config.use_window_attn)
        assert len(config.window_attn) == config.vision_config.num_hidden_layers
        # for idx, use in enumerate(config.window_attn):
        #     if use:
        #         print(f'replace the layer {idx} with WindowBLIP2EncoderLayer')
        #         self.vision_model.encoder.layers[idx] = WindowBLIP2EncoderLayer(config.vision_config, window_size=config.window_size)
    ...
    def extract_image_feature(
        ...
        language_model_inputs = self.extract_feature(
            ...
            boxes_mask=boxes_mask,  # torch.Size([2])
    ...
    def lm_forward(
        ...
        inputs_embeds = self.language_model.get_input_embeddings()(input_ids)
        language_model_inputs = self.extract_feature(pixel_values=pixel_values, boxes=boxes, boxes_mask=boxes_mask)
    ...
    def generate(
        ...
        if hasattr(outputs, 'sequences_scores'):
            scores = outputs.sequences_scores.exp()
        else:
            # TODO: bug here
            scores = torch.stack(outputs.scores, dim=1)
            scores = torch.gather(
                input=scores.to(torch.float32).log_softmax(-1),
                index=sequences[:, 1:].unsqueeze(-1),
                dim=2
            ).squeeze()
            scores_mask = (sequences[:, 1:] != self.config.text_config.eos_token_id)
            scores = torch.where(scores_mask, scores, 0).sum(1) / scores_mask.sum(1)
            scores = scores.exp()
            # TODO: hardcode
    ...
    def lm_generate_forward_chunk(self, pixel_values, boxes, boxes_mask, input_ids, attention_mask):
        return self(
            pixel_values=pixel_values,
            boxes=boxes,
            boxes_mask=boxes_mask,
